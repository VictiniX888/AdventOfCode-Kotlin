...
class Grid(private val initVal: Int) {
    ...
    fun set(value: Int, point: MutablePoint) = set(value, point.x, point.y)

    fun get(x: Int, y: Int): Int =
        if (x in xRange && y in yRange) gridList[(y-yRange.first)*width() + (x-xRange.first)] else initVal

    fun get(point: MutablePoint): Int = get(point.x, point.y)

    fun pointOf(element: Int): MutablePoint? {
        val index = gridList.indexOf(element)
        return if (index == -1) {
            null
        } else {
            MutablePoint(index % width() + xRange.first, index / width() + yRange.first)
        }
    }
